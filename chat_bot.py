...
uploaded_file = st.file_uploader("Upload chatbot.txt file", type=['txt'])

if uploaded_file is not None:
    # Read the uploaded file
    raw = uploaded_file.read().decode('utf8').lower()
    ...
    if st.button("Send") and user_input:
        if user_input.lower() == 'bye':
            ...
            if greeting(user_input) is not None:
                ...
                sent_tokens.remove(user_input)  # Remove the user's input to prevent duplicate processing
    ...
    st.warning("Please upload the 'chatbot.txt' file to start the conversation.")
